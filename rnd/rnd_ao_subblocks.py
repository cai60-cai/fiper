...
class Predictor(nn.Module):
    def __init__(self, base_network, in_features=4096):
        super().__init__()
    ...
    def forward(self, input, global_cond=None):
        # Call the base network with additional argument
        output = self.base_network(input, global_cond)
        # Pass through the additional layers
        return self.additional_layers(output)

class Target(nn.Module):
    def __init__(self, base_network, in_features=4096):
        super().__init__()
        self.base_network = base_network
        self.additional_layer = nn.Linear(in_features=in_features, out_features=256)

    def forward(self, input, global_cond=None):
        # Call the base network with additional argument
        output = self.base_network(input, global_cond)
        # Pass through the additional layer
        return self.additional_layer(output)